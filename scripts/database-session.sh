...
ACTIVE_SESSION=$(oci bastion session list \
    ...
    --all \
    --output json |
    jq -r --arg DATABASE_OCID "$DATABASE_OCID" '
        .data[] 
        | select(.["target-resource-details"].["session-type"] == "PORT_FORWARDING") 
        | select(.["target-resource-details"].["target-resource-id"] == $DATABASE_OCID) 
        | @json
    ' | head -n 1
)
