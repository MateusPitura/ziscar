...
export abstract class AccountReceivableInstallmentRepository {
  ...
  abstract addPaymentMethodToInstallment(installmentId: string, data: createPaymentMethodToInstallment): Promise<AccountReceivableInstallment>
  abstract findById(id: string): Promise<AccountReceivableInstallment | null>;
  abstract findAllByAccountReceivableId(accountReceivableId: string, dueDate: Date): Promise<AccountReceivableInstallmentPayload[]>;
  abstract findPaymentMethodByInstallmentId(installmentId: string): Promise<AccountReceivableInstallment | null>;
